...
function synchronize_tt!(tt::Vector{Array{V, N}}; subcomm = nothing, juliasource::Int = 1) where {V, N}
    if !MPI.Initialized()
        println("Warning! synchronize_tt has been called, but MPI is not initialized, please use TCI.initializempi() before contract() and use TCI.finalizempi() afterwards")
    else
        if subcomm != nothing
        ...
        if juliarank == juliasource
            for j in 1:nprocs
                if j != juliarank
                    push!(reqs, MPI.isend(tt[1:end], comm; dest=j-1, tag=juliarank))
                end
            end
        end
        if juliarank != juliasource
            tt[1:end] = MPI.recv(comm; source=juliasource-1, tag=juliasource)
        end
        MPI.Waitall(reqs)
    end
end